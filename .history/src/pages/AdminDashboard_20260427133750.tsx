  ...
  const fetchSold = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        profiles:profiles!tickets_purchased_by_fkey(full_name, email)
      `)
      .eq('status', 'sold');
    setSoldTickets(data || []);
...
const grouped = Object.values(
  ...
  const soldGrouped = Object.values(
    soldTickets.reduce((acc, ticket) => {
      ...
      acc[key].tickets.push(ticket);
      return acc;
    }, {} as Record<string, { profile: any; tickets: any[] }>)
  );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <p className="text-gray-500">No hay boletos pendientes de validación</p>
          </motion.div>
        ) : (
