...
import {
  CreditCard, Upload, MessageCircle, CheckCircle, Clock,
  AlertCircle, X, FileText, Image as ImageIcon
} from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const fetchReserved = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('tickets')
      .select('id, ticket_number, reservation_expires_at, raffle_id, raffles(name, ticket_price)')
  ...
  const earliestExpiry = reservedTickets.length > 0
    ? reservedTickets.reduce((min, t) => t.reservation_expires_at < min ? t.reservation_expires_at : min, reservedTickets[0].reservation_expires_at)
    : null;
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (!validTypes.includes(file.type)) { setError('Formato invalido. Usa JPG, PNG, WebP o PDF.'); return; }
    ...
    try {
      ...
      for (const ticket of reservedTickets) {
        await supabase.from('receipts').insert({
          ...
          file_url: urlData.publicUrl, file_name: receiptFile.name, file_type: receiptFile.type,
        });
      }
      const ticketIds = reservedTickets.map((t) => t.id);
      await supabase.from('tickets').update({ status: 'pending' }).in('id', ticketIds);
      playSuccess(); setSuccess(true);
    } catch (err) { setError('Error al subir comprobante. Intenta de nuevo.' + (err as Error).message); }
    finally { setUploading(false); }
  };

  const handleWhatsApp = () => {
    const msg = encodeURIComponent(
      `Hola! Mi nombre es ${profile?.full_name || user?.email}. Reserve los boletos #${ticketNumbers} para "${raffleName}". Adjunto mi comprobante de pago.`
    );
  ...
  const handleExpired = useCallback(async () => {
    ...
    await supabase.from('tickets')
      ...
      .eq('reserved_by', user.id).eq('status', 'reserved')
  ...
  return (
    <div className="min-h-screen pt-16 pb-20 px-4" style={{ background: '#101010' }}>
        ...
        <AnimatePresence mode="wait">
          {success ? (
            ...
            <motion.div key="checkout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="rounded-2xl border border-gray-800 p-6" style={{ background: '#141414' }}>
                ...
                <div className="space-y-2">
                  {reservedTickets.map((t) => (
                    <div key={t.id} className="flex items-center justify-between p-3 rounded-lg" style={{ background: '#1a1a1a' }}>
                      <span className="text-white font-semibold">#{t.ticket_number}</span>
                      <span className="font-semibold" style={{ color: GOLD }}>${ticketPrice.toLocaleString()} MXN</span>
                    </div>
                  ))}
                </div>
                <div className="mt-4 pt-4 border-t border-gray-800 flex justify-between">
                  <span className="text-white font-bold">Total</span>
                  <span className="font-bold text-lg" style={{ color: GOLD }}>${totalAmount.toLocaleString()} MXN</span>
                </div>
              </div>
                ...
                <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
                  ...
                  {receiptFile ? (
                    <div className="space-y-3">
                      ...
                      <p className="text-white text-sm font-medium">{receiptFile.name}</p>
                      <button onClick={(e) => { e.stopPropagation(); setReceiptFile(null); setReceiptPreview(null); }}
                        className="text-red-400 text-xs hover:underline"><X size={14} className="inline mr-1" />Eliminar</button>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <ImageIcon size={40} className="mx-auto text-gray-600" />
                      <p className="text-gray-400 text-sm">Arrastra tu comprobante aqui o haz clic para seleccionar</p>
                      <p className="text-gray-600 text-xs">JPG, PNG, WebP o PDF (max 10MB)</p>
                    </div>
                  )}
                </div>
                {error && <div className="flex items-center gap-2 mt-3 text-red-400 text-sm"><AlertCircle size={14} />{error}</div>}
                <button onClick={handleUpload} disabled={!receiptFile || uploading}
              ...
              <div className="rounded-2xl border border-gray-800 p-6" style={{ background: '#141414' }}>
                <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  <MessageCircle size={18} className="text-green-500" /> Confirmar por WhatsApp
                </h3>
                <p className="text-gray-400 text-sm mb-4">Tambien puedes enviar tu comprobante por WhatsApp para validacion mas rapida.</p>
                <button onClick={handleWhatsApp}
                  ...
                  <MessageCircle size={18} />{whatsappSent ? 'Abrir WhatsApp de nuevo' : 'Enviar por WhatsApp'}
                </button>
                {whatsappSent && <p className="text-green-400 text-xs mt-2 text-center">Mensaje preparado. No olvides adjuntar tu comprobante.</p>}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
