...
interface PendingTicket {
  ...
  receipts: { file_url: string; file_name: string; file_type: string; created_at: string }[];
}
      ...
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-gray-500 mb-8">
            {pendingTickets.length} boleto{pendingTickets.length !== 1 ? 's' : ''} pendiente{pendingTickets.length !== 1 ? 's' : ''} de validacion
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <h2 className="text-xl font-bold text-white mb-2">Todo al dia</h2>
            <p className="text-gray-500">No hay boletos pendientes de validacion</p>
          </motion.div>
        ) : (
                            ...
                            <div className="flex flex-wrap gap-2">
                              {ticket.receipts.map((receipt, ri) => (
                                <button key={ri} onClick={() => setPreviewUrl(receipt.file_url)}
                                  ...
                                  {receipt.file_type?.startsWith('image/') ? <ImageIcon size={12} /> : <FileText size={12} />}
      ...
      <AnimatePresence>
        {previewUrl && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ background: 'rgba(0,0,0,0.9)' }} onClick={() => setPreviewUrl(null)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              className="max-w-3xl max-h-[80vh] overflow-auto rounded-xl" onClick={(e) => e.stopPropagation()}>
              {previewUrl.endsWith('.pdf') ? (
                <iframe src={previewUrl} className="w-full h-[80vh] rounded-xl" title="Receipt" />
              ) : (
