...
export default function LandingPage() {
  ...
  const fetchData = async () => {
    ...
    if (raffleData) {
      ...
      if (ticketData) {
        ...
        setSoldCount(ticketData.filter((t) => t.status === 'sold' || t.status === 'pending').length);
      }
    }
    setLoading(false);
  };
  ...
  useEffect(() => { fetchData(); fetchUserReservation(); }, [fetchUserReservation]);

  useEffect(() => {
    const interval = setInterval(() => { fetchData(); fetchUserReservation(); }, 10000);
    ...
    <div className="min-h-screen" style={{ background: '#101010' }}>
      ...
      <section className="relative pt-20 pb-16 px-4 overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl" style={{ background: `${GOLD}15` }} />
          <div className="absolute bottom-1/4 right-1/4 w-64 h-64 rounded-full blur-3xl" style={{ background: '#2E7D3215' }} />
        </div>
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold mb-6"
              style={{ background: `${GOLD}15`, color: GOLD, border: `1px solid ${GOLD}30` }}>
              <Sparkles size={14} /> Sorteo en vivo
            </div>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-4 leading-tight">
              {raffle?.name || 'Sorteo de Licores'}<br />
              <span style={{ color: GOLD }}>Premium</span>
            </h1>
            <p className="text-gray-400 text-lg max-w-xl mx-auto mb-8">
              {raffle?.description || 'Participa por las mejores marcas. Selecciona tus numeros favoritos y gana.'}
            </p>
          </motion.div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {prizes.map((prize) => (
            <PrizeCard key={prize.id} tier={prize.tier} name={prize.name}
              description={prize.description} imageUrl={prize.image_url}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-400">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded border-2" style={{ borderColor: GOLD }} /> Disponible
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded" style={{ background: 'rgba(255,193,7,0.3)', border: '2px solid #FFC107' }} /> Reservado
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded" style={{ background: 'rgba(255,152,0,0.3)', border: '2px solid #FF9800' }} /> Pendiente
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded" style={{ background: 'rgba(244,67,54,0.3)', border: '2px solid #F44336' }} /> Vendido
              </span>
            </div>
          </div>

          <TicketGrid tickets={tickets} selectedTickets={selectedTickets}
            onToggle={handleToggleTicket} currentUserId={user?.id ?? null} />
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              {user ? (
                ...
                <button onClick={() => navigate('/login')}
                  ...
                  Inicia sesion para reservar <ArrowRight size={16} />
                </button>
              )}
            </motion.div>
          )}
