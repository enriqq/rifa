...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: tickets, error: ticketsError } = await adminClient
      .from("tickets")
      .select("id, ticket_number, raffle_id, reserved_by, raffles!inner(name, ticket_price), profiles:reserved_by(id), receipts(file_url, file_name, file_type, created_at)")
      .eq("status", "pending")
      .order("ticket_number");
