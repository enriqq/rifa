...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!ticketIds || !Array.isArray(ticketIds) || ticketIds.length === 0 || !action) {
      return new Response(JSON.stringify({ error: "Missing ticketIds or action" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "approve") {
      ...
      const { error } = await adminClient
        ...
        .update({
          status: "sold",
          purchased_by: tickets?.reserved_by,
          purchased_at: new Date().toISOString(),
        })
        .eq("id", ticketIds);

      // Busca el email del usuario
      const { data: userData } = await adminClient.auth.admin.getUserById(tickets?.reserved_by);
