...
interface UserTicket {
  ...
  prizes: { name: string; ticket_price: number; tier: number };
}
...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      const { data } = await supabase
        .from('tickets')
        .select('id, ticket_number, status, prize_id, reservation_expires_at, prizes(name, ticket_price, tier)')
  ...
  const grouped = tickets.reduce<Record<string, UserTicket[]>>((acc, t) => {
    const key = t.prize_id;
    if (!acc[key]) acc[key] = [];
    acc[key].push(t);
    return acc;
  }, {});
    ...
    <div className="min-h-screen pt-20 pb-20 px-4" style={{ background: '#101010' }}>
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-gray-500 mb-8">Estado en tiempo real de tus tickets reservados</p>
        </motion.div>
        ...
        {tickets.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-xl font-bold text-white mb-2">No tienes tickets</h2>
          ...
          <div className="space-y-6">
            {Object.entries(grouped).map(([prizeId, prizeTickets]) => (
              <div
                key={prizeId}
              ...
              >
                <div className="p-5 border-b border-gray-800">
                  <h3 className="text-lg font-semibold text-white">
                    {prizeTickets[0]?.prizes?.name}
                  </h3>
                  <p className="text-gray-500 text-sm">
                    ${prizeTickets[0]?.prizes?.ticket_price?.toLocaleString()} CLP por ticket
                  </p>
                </div>
                <div className="divide-y divide-gray-800/50">
                  {prizeTickets.map((ticket, i) => {
